...
parser.add_argument("--learning_rate", type=float, default=3e-3) #5e-3
parser.add_argument("--weight_decay", type=float, default=3e-3) #2e-3
...
# target_data = target_data.to(device)  # 不再需要放到 device 上
...
def train(epoch):
    ...
    # 只用 source 域 loss
    loss = cls_loss + (float(epoch) / epochs) * semi_loss
