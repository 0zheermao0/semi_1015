...
class MoE(nn.Module):
    ...
    def __init__(self, input_size, output_size, num_experts, noisy_gating=False, k=3, coef=1e-2, gnn_type='gcn', num_hops=None):
        ...
        for i in range(self.num_experts):
            ...
            if num_hops[i] > 1:
                layers = []
                layers.append(expert)
                for _ in range(num_hops[i] - 1):
                    if gnn_type == 'gcn':
                        layers.append(GCNConv(output_size, output_size, normalize=False))
                    elif gnn_type == 'sage':
                        layers.append(SAGEConv(output_size, output_size, normalize=False))
                    elif gnn_type == 'ppmi':
                        layers.append(PPMIConv(output_size, output_size))
    ...
    def get_expert_output(self, expert_idx, x, edge_index, edge_attr=None):
        ...
        if isinstance(expert, nn.Sequential):
            ...
            return expert(x, edge_index, edge_attr)
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        for i in range(self.num_experts):
            ...
            if isinstance(expert, nn.Sequential):
                # For Sequential, we need to handle each layer manually if necessary
                output = x
                for layer in expert:
                    if isinstance(layer, (GCNConv, SAGEConv, PPMIConv)):
                        output = layer(output, edge_index, edge_attr)
                        ...
                        output = layer(output)
                expert_i_output = output
            else:
                expert_i_output = expert(x, edge_index, edge_attr)
...
#     num_hops: list of integers - number of hops for each expert (for multi-hop configurations)
#     """
#     def __init__(self, input_size, output_size, num_experts, noisy_gating=True, k=3, coef=1e-2, gnn_type='gcn', num_hops=None):
#         super(MoE, self).__init__()
#         self.noisy_gating = noisy_gating
#         self.num_experts = num_experts
#         self.k = k
...
#         # with torch.no_grad():
#         # 获取gates和load
#         gates, _ = self.noisy_top_k_gating(x, edge_index, train=False)

#         # 使用传入的k或默认的self.k
#         k = k if k is not None else self.k
        
#         # 获取每个节点的top-k专家索引和对应的概率
#         expert_probs, expert_indices = gates.topk(k, dim=1)
...
#         encourages all experts to be approximately equally used across a batch.
#         """
#         gates, load = self.noisy_top_k_gating(x, edge_index, self.training)
#         # calculate importance loss
#         importance = gates.sum(0)
#         #
#         loss = self.cv_squared(importance) + self.cv_squared(load)
